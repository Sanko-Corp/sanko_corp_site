...
給排水・消火・空調の配管工事の事業紹介、品質管理方針、採用情報、お問い合わせを掲載しています。
...
│   ├── js/main.js         モバイルナビ開閉・お問い合わせフォーム
│   └── images/            写真素材
├── docs/
...
ブラウザで `index.html` を直接開くだけでも表示できますが、お問い合わせページの
...
- **お問い合わせフォーム**: 現状はデザイン確認用のモックで、実送信は行いません
  （`assets/js/main.js` 参照）。本番ではフォーム送信のバックエンド（メール送信API・
  reCAPTCHA 等）の実装が必要です。
- **写真**: `assets/images/` の写真は配管・施工イメージです。自社の実写真への
  差し替えを推奨します。
